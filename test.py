...
class ApexDataWidget(QtWidgets.QDialog):
    ...
    def setupTopFrame(self):
        ...
        self.top_frame_layout_horizontal = QtWidgets.QHBoxLayout(self.top_frame)
    ...
    def setupWindowButtonsFrame(self):
        ...
        self.close_button = QtWidgets.QPushButton(self.window_buttons_frame, clicked=self.close)
    ...
    def createStatsFrame(self):
        ...
        stats_frame_layout_horizontal.addWidget(self.createStatsBox())
        stats_frame_layout_horizontal.addWidget(self.createStatsBox())
        stats_frame_layout_horizontal.addWidget(self.createStatsBox())
        stats_frame_layout_horizontal.addWidget(self.createStatsBox())
        return stats_frame
    ...
    def plot_graph(self):
        ...
        self.account_data['EntryT'] = pd.to_datetime(self.account_data['EntryT'])
    ...
    def addLines(self, ax):
        if not self.account_row.empty:
            ...
            if target_condition == 'On':
                target_line, stop_line, _, _ = self.calculate_target_and_stop_lines()
                if target_line is not None:
                    ax.axhline(target_line, color='green', linestyle='--', linewidth=1, label='Target')
                if stop_line is not None:
                    ax.axhline(stop_line, color='red', linestyle='--', linewidth=1, label='Stop Loss')

    def plot_trade_performance(self, data, aggregation='single'):
        self.clearLayout(self.second_graph_frame.layout())
        filtered_data = data.copy()
        filtered_data['EntryT'] = pd.to_datetime(filtered_data['EntryT'], errors='coerce')
        ...
        grouped_data = self.aggregate_trade_data(filtered_data, aggregation)
    ...
    def aggregate_trade_data(self, data, aggregation):
        if aggregation == 'daily':
            return data.groupby(data['EntryT'].dt.date).agg(
                Profit=('Profit', 'sum'),
                Qty=('Qty', 'sum')).reset_index()
        elif aggregation == 'weekly':
            return data.groupby(pd.Grouper(key='EntryT', freq='W-MON')).agg(
                Profit=('Profit', 'sum'),
                Qty=('Qty', 'sum')).reset_index()
        else:
            return data.groupby('EntryT').agg(
                Profit=('Profit', 'sum'),
                Qty=('Qty', 'sum'),
                LorS=('LorS', 'first'),
    ...
    def plot_calendar(self):
        self.clearLayout(self.third_graph_frame.layout())
    ...
    def open_settings_widget(self):
        if self.accountselection_comboBox.currentIndex() != -1:
